...
``tai_channel_twilio`` is on the allowlist. The rule (see the README): the shipped
package imports
...
The runtime walk imports ``tai_channel_twilio`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_channel_twilio`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_channel_twilio"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_channel_twilio`` graph pulls in -- the
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_channel_twilio graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_channel_twilio module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_channel_twilio sources: {offenders}"
